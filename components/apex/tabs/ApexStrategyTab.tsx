...
import type { ApexStrategyBoard as ApexStrategyBoardData } from "@/lib/apex/types"
import type { ApexBehaviorSignals } from "@/lib/apex/behavior"

export type ApexStrategyTabProps = {
  board: ApexStrategyBoardData | null
  isLoading: boolean
  error: string | null
...
export function ApexStrategyTab({
  board,
  isLoading,
  error,
  behaviorSignals,
  behaviorLoading,
}: ApexStrategyTabProps) {
  ...
  async function handleGenerateStrategy() {
    ...
    try {
      const response = await fetch("/api/apex/strategy", {
        method: "POST",
        headers: {
      ...
      <section className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">
            Apex turns your current signals into a simple plan: what to focus on, what to avoid, and what to do next.
          </p>
        </div>
                ...
                <div className="space-y-3">
                  {isLoading ? (
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-xs leading-5 text-slate-500">
                              {generatedStrategy ? "Generated by Strategy Mode." : "Priority generated from your current Apex board."}
                            </p>
                          </div>
                        </div>
                      </article>
                    ...
                    <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5 text-sm text-slate-500">
                      Apex needs more saved jobs, applications, or resume context to build a stronger focus list.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {isLoading ? (
                    ...
                    <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5 text-sm text-slate-500">
                      No execution moves yet. Add or save jobs so Apex can build the plan.
                    </div>
                  )}
                </div>
              </div>
