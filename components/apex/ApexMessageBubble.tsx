...
const RECOMMENDATION_CONFIG = {
  ...
  Explore: { bg: "bg-slate-500",  pill: "bg-slate-50 text-slate-700 border-slate-200" },
} as const
...
export function ApexMessageBubble({ response, context = "dashboard", compact = false, onUpgrade }: Props) {
  ...
  return (
    <div className={`group flex items-start ${isCompact ? "gap-2.5" : "gap-3"}`}>
      {/* Avatar */}
      <div
        className={`mt-0.5 flex-shrink-0 inline-flex items-center justify-center rounded-lg bg-slate-600 shadow-[0_2px_8px_rgba(99,102,241,0.30)] ${
          ...
          {showGatedUpgradeCard && response.gated && (
            ...
            >
              <div className="flex items-start gap-3">
                <div className="mt-0.5 inline-flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-slate-100">
                ...
                <div className="flex-1">
                  ...
                  <button
                    ...
                    className="mt-2 inline-flex items-center gap-1.5 rounded-lg bg-slate-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-slate-700"
                  >
