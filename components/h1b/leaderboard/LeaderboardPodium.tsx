...
const MEDAL: Record<number, { ring: string; text: string; label: string }> = {
  1: { ring: "ring-amber-300", text: "text-amber-600", label: "Gold" },
  2: { ring: "ring-slate-300", text: "text-slate-500", label: "Silver" },
  3: { ring: "ring-orange-300", text: "text-orange-600", label: "Bronze" },
}
...
function PodiumCard({
  ...
  return (
    <div
      className={
        "relative flex flex-col rounded-2xl border p-5 transition-transform duration-300 " +
        (featured
          ? "border-orange-300 bg-orange-50/60 shadow-[0_0_0_1px_rgba(255,92,24,0.12)] sm:-translate-y-3"
    ...
    >
      <div className="flex items-start gap-3">
        <CompanyLogo
          ...
          className={"h-12 w-12 shrink-0 rounded-xl ring-1 " + medal.ring}
          priority={priority}
        />
        ...
        <span
          className={
            "inline-flex items-center gap-1 rounded-full bg-slate-50 px-2 py-1 text-[11px] font-semibold ring-1 ring-inset ring-slate-200 " +
            medal.text
          }
        >
