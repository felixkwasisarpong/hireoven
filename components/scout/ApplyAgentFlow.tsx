...
function applyTailoredSignalsToResume(base: Resume, job: JobState | null): Resume {
  ...
  for (const suggestion of suggestions) {
    ...
    for (const exp of next.work_experience ?? []) {
      ...
      if (normalizePreviewText(description).includes(original)) {
        exp.description = description.replace(suggestion.original, suggestion.suggested)
        replaced = true
        break
      }
    }
...
export function ApplyAgentFlow({ initialJobs, extensionConnected = false, onFollowUp, onDone }: Props) {
  ...
  const autoSidePreviewForJobRef = useRef<string | null>(null)
  ...
  useEffect(() => {
    ...
    if (!reviewJob?.jobId) return
    if (!reviewResumePreview) {
      void ensurePrimaryResume()
      return
    }
    if (autoSidePreviewForJobRef.current === reviewJob.jobId) return

    autoSidePreviewForJobRef.current = reviewJob.jobId
    setSidePreview({
      kind: "resume",
      resume: reviewResumePreview,
      title: `Tailored resume · ${reviewJob.jobTitle ?? "Job"}`,
    })
  }, [ensurePrimaryResume, phase, reviewJob?.jobId, reviewJob?.jobTitle, reviewResumePreview])
    ...
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      {phase === "reviewing" && reviewJobIdx !== null && (
        <div className="mx-4 mt-3 mb-1 rounded-xl border border-orange-200 bg-orange-50/30 px-4 py-3">
          ...
          {(reviewJob?.prepResult?.suggestedSummaryRewrite || (reviewJob?.prepResult?.bulletSuggestionsPreview?.length ?? 0) > 0) && (
            <div className="mt-2 rounded-lg border border-slate-200 bg-white p-3">
              <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-slate-500">Tailor preview</p>
              {reviewJob?.prepResult?.suggestedSummaryRewrite && (
                <div className="mt-1">
                  <p className="text-[10px] font-semibold text-slate-600">Suggested summary direction</p>
                  <p className="text-[11px] text-slate-700">{reviewJob.prepResult.suggestedSummaryRewrite}</p>
                </div>
              )}
              {(reviewJob?.prepResult?.bulletSuggestionsPreview?.length ?? 0) > 0 && (
                <div className="mt-2 space-y-2">
                  {reviewJob?.prepResult?.bulletSuggestionsPreview?.map((bullet, idx) => (
                    <div key={`${reviewJob.jobId}-preview-bullet-${idx}`} className="rounded-md border border-slate-100 bg-slate-50 p-2">
                      <p className="text-[10px] font-semibold text-slate-500">Before</p>
                      <p className="text-[11px] text-slate-700">{bullet.original}</p>
                      <p className="mt-1 text-[10px] font-semibold text-slate-500">Suggested</p>
                      <p className="text-[11px] text-slate-700">{bullet.suggested}</p>
                      <p className="mt-1 text-[10px] text-slate-500">{bullet.reason}</p>
                    </div>
                  ))}
                </div>
              )}
