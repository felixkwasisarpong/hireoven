    ...
    <div className="space-y-6">
      ...
      {(hasConversation || isLoading || error) && (
        <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
          <div className="flex items-center justify-between gap-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 via-white to-orange-50/70 px-5 py-4">
            <div className="flex items-center gap-3">
              <span className="inline-flex h-9 w-9 items-center justify-center rounded-2xl bg-orange-600 text-white shadow-[0_8px_18px_rgba(234,88,12,0.22)]">
            ...
            {hasConversation && (
              <button
                type="button"
                onClick={onClearChat}
                className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-500 shadow-sm transition hover:bg-slate-50 hover:text-slate-700"
              >
          ...
          <div className="max-h-[640px] space-y-5 overflow-y-auto bg-slate-50/60 p-4 sm:p-5">
            {messages.map((msg) =>
              msg.role === "user" ? (
                <div key={msg.id} className="flex justify-end">
                  <div className="max-w-[82%] rounded-[22px] rounded-tr-md bg-slate-950 px-4 py-3 text-sm leading-6 text-white shadow-[0_10px_24px_rgba(15,23,42,0.16)]">
                    {msg.text}
                  </div>
                </div>
              ) : (
