...
function RangeBar({ a }: { a: WageAggregate }) {
  ...
  return (
    <div className="mt-4">
      <div className="relative h-2 w-full bg-[#0a0e0c] border border-[rgba(120,200,160,0.2)]">
        <div
          className="absolute h-full bg-[#f5a623]/40"
          style={{ left: `${left}%`, width: `${Math.max(width, 2)}%` }}
        />
        <div
          className="absolute top-1/2 h-3 w-1 -translate-y-1/2 bg-[#38e08a]"
          style={{ left: `${mid}%` }}
        />
      </div>
      <div className="mt-1 flex justify-between text-xs text-[#ccd6cf]/55 tabular-nums">
...
export function SalaryCard({
  ...
  return (
    <section className="term-panel p-6 sm:p-8">
      <p className="text-sm text-[#ccd6cf]/55">{subtitle}</p>
      <h1 className="text-xl font-semibold tracking-tight text-white sm:text-2xl">{title}</h1>

      {aggregate ? (
        <>
          <div className="mt-5 flex items-end gap-3">
            <div className="text-5xl font-semibold tabular-nums text-[#38e08a]">
              {fmtUsd(aggregate.p50)}
            </div>
            <div className="pb-1 text-sm text-[#ccd6cf]/55">
              median · n={aggregate.n.toLocaleString()}
            </div>
          </div>
          <RangeBar a={aggregate} />
          <p className="mt-4 text-xs text-[#ccd6cf]/45">
