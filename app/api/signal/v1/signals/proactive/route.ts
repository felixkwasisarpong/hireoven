...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [
      ...
      pool.query<HighMatchRow>(
        `SELECT *
         ...
         LIMIT 12`,
        [auth.subjectUserId, auth.tenantId]
      ),
    ...
    return finish(signalApiJson(auth, { snapshot }))
  } catch (error) {
    console.error("[signal/v1/signals/proactive] error", error)
    return finish(signalApiError(500, "Unable to load proactive signals", "INTERNAL_ERROR", auth.requestId, auth.rateLimit, undefined, auth.quota))
  }
}
