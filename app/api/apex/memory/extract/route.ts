 ...
 *
 * Called on first Apex session and on-demand from the Memory panel.
...
import { extractFromBehaviorSignals } from "@/lib/apex/memory/extractor"
import { persistCandidates, getMemories } from "@/lib/apex/memory/store"
import { MAX_MEMORIES_PER_USER } from "@/lib/apex/memory/types"
...
export async function POST() {
  ...
  const signals = await getApexBehaviorSignals(user.id).catch(() => null)
  const candidates = signals ? extractFromBehaviorSignals(signals) : []

  const written = await persistCandidates(user.id, pool, candidates)

  console.log("[apex:memory:extract]", { userId: user.id, candidates: candidates.length, written })
