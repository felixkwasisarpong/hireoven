...
function buildJdInsights(typedJobs: JobWithCompany[]): JdInsights {
...
export default function CompanyProfilePageClient({
  companyId,
  initialCompany,
  initialRecords,
  initialLcaStats,
  ...
  const [records,     setRecords]     = useState<H1BRecord[]>(initialRecords)
  const [lcaStats,    setLcaStats]    = useState<EmployerLCAStats | null>(initialLcaStats)
  const [jobs,        setJobs]        = useState<JobWithCompany[]>(initialJobs)
  const [newThisWeek, setNewThisWeek] = useState(initialNewThisWeek)
  const [jdInsights,  setJdInsights]  = useState<JdInsights | null>(initialJdInsights)
    ...
    async function load() {
      try {
        ...
        const [companyRes, h1bRes, jobsRes] = await Promise.all([
          ...
          fetch(`/api/h1b/records?companyId=${encodeURIComponent(companyId)}&limit=6`),
          fetch(`/api/jobs?company_id=${encodeURIComponent(companyId)}&limit=50&sort=fresh`),
        ])
        ...
        const jobsFromCompanyRoute = (companyData?.jobs ?? []).map((job) => ({
          ...job,
          company: companyData?.company ?? job.company,
        }))
        const typedJobs = jobsFromCompanyRoute.length > 0 ? jobsFromCompanyRoute : jobsPayload.jobs
        setJobs(typedJobs)
        setNewThisWeek(
          typedJobs.filter((j) => new Date(j.first_detected_at).getTime() >= weekStart.getTime()).length
        )
        setJdInsights(buildJdInsights(typedJobs))
      } finally {
        setIsLoading(false)
      }
    }
            ...
            <div className="space-y-6">
              <div className="surface-card overflow-hidden">
                ...
                <div className="border-t border-gray-100 px-6 py-6">
                  ...
                  <SimilarCompanies companyId={companyId} industry={company.industry} limit={4} />
                </div>
              </div>
            </div>
          )}
          ...
          {tab === "about" && (
            <div className="space-y-8">
              ...
              <div className="border-t border-gray-100 pt-8">
                <EmployerHealthScore companyId={companyId} companyName={company.name} />
              </div>

              {/* Sponsorship Truth Score */}
              <div className="border-t border-gray-100 pt-8">
                <SponsorshipTruthScore companyId={companyId} companyName={company.name} />
              </div>
