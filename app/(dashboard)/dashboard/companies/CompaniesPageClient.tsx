...
export default function CompaniesPageClient({
  ...
  initialTotal,
  initialCompaniesLoaded,
  initialIndustriesLoaded,
  initialNewTodayLoaded,
  ...
  const [all,          setAll]          = useState<Company[]>(initialCompanies)
  const [industries,   setIndustries]   = useState<string[]>(initialIndustries)
  const [newToday,     setNewToday]     = useState<Record<string, number>>(initialNewTodayByCompanyId)
  const [total,        setTotal]        = useState(initialTotal)
  const [isLoading,    setIsLoading]    = useState(!initialCompaniesLoaded)
  ...
  const selectedIndustriesKey = selectedIndustries.join(",")
  ...
  useEffect(() => {
    if (initialCompaniesLoaded || offset > 0) return
    async function loadInitial() {
      ...
      if (selectedIndustries.length) params.set("industry", selectedIndustriesKey)
      if (selectedSizes.length) params.set("size", selectedSizesKey)
    ...
    void loadInitial()
  }, [initialCompaniesLoaded, offset, selectedIndustriesKey, selectedSizesKey, selectedAts, sponsorsH1b, hasJobs, sort, qParam])
  ...
  useEffect(() => {
    ...
    void loadMore()
  }, [offset, selectedIndustriesKey, selectedSizesKey, selectedAts, sponsorsH1b, hasJobs, sort, qParam])

  useEffect(() => {
    if (initialIndustriesLoaded) return
  ...
  useEffect(() => {
    if (initialNewTodayLoaded) return
