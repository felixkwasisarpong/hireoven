...
export default function AlertsPageClient({
  ...
  const [alertsLoaded, setAlertsLoaded] = useState(initialAlertsLoaded)
  const [companies, setCompanies] = useState<Company[]>(initialCompanies)
  const [companiesLoaded, setCompaniesLoaded] = useState(initialCompaniesLoaded)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [companySearch, setCompanySearch] = useState("")
  const [draft, setDraft] = useState<AlertDraft>(emptyDraft(initialAlertFrequency))

  const isLoading = !alertsLoaded || !companiesLoaded

  useEffect(() => {
    const frequency = profile?.alert_frequency ?? initialAlertFrequency
    setDraft((current) =>
      current.name ||
      current.keywords ||
      current.locations ||
      current.seniority.length > 0 ||
      current.remoteOnly ||
      current.sponsorshipRequired ||
      current.companyIds.length > 0
        ? current
        : { ...current, frequency }
    )
  }, [profile?.alert_frequency, initialAlertFrequency])

  useEffect(() => {
    if (!companiesLoaded) {
      fetch("/api/companies?limit=50&sort=job_count")
        .then((res) =>
          res.ok ? (res.json() as Promise<{ companies: Company[] }>) : null
        )
        .then((data) => {
          if (!data) return
  ...
  useEffect(() => {
    if (alertsLoaded || authLoading) return
    if (!user?.id) {
      setAlerts([])
  ...
  }, [alertsLoaded, authLoading, user?.id])
  ...
  const quickAlertTemplates = useMemo<AlertTemplate[]>(
    () => [
      {
        id: "remote-sponsored",
        title: "Remote + sponsorship",
        description:
          "Catch remote engineering roles that explicitly require sponsorship support.",
        draft: {
          name: "Remote sponsored engineering",
      ...
      },
      {
        ...
        description:
          "Track startup roles with ownership-heavy product and engineering work.",
        draft: {
          name: "Founding builder roles",
          keywords: "founding engineer, founding product manager, early stage",
      ...
      {
        id: "junior-launch",
        title: "Junior launch roles",
        description:
          "Stay on top of intern and junior openings with frequent updates.",
        draft: {
          name: "Junior + intern roles",
          keywords: "intern, new grad, junior software engineer",
  ...
  function startFromTemplate(template: AlertTemplate) {
    const profileAlertFrequency = profile?.alert_frequency ?? initialAlertFrequency
    setDraft({
      ...emptyDraft(profileAlertFrequency),
      ...template.draft,
      frequency: template.draft.frequency ?? profileAlertFrequency,
      seniority: template.draft.seniority ?? [],
      companyIds: template.draft.companyIds ?? [],
    })
    setCompanySearch("")
    setIsModalOpen(true)
  }
  ...
  async function handleCreateAlert() {
    ...
    const profileAlertFrequency = profile?.alert_frequency ?? initialAlertFrequency
    if (draft.frequency !== profileAlertFrequency) {
        ...
        {alerts.length === 0 && !isLoading ? (
          <section className="empty-state py-12">
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              {quickAlertTemplates.map((template) => (
                <button
                  key={template.id}
                  type="button"
                  onClick={() => startFromTemplate(template)}
                  className="rounded-3xl border border-gray-200 bg-white p-4 text-left transition hover:border-[#FFD2B8] hover:bg-[#FFF7F2]"
                >
                  <p className="text-sm font-semibold text-gray-900">{template.title}</p>
                  <p className="mt-1.5 text-xs leading-5 text-gray-500">
                    {template.description}
                  </p>
                  <span className="mt-3 inline-flex items-center rounded-full bg-[#FFF7F2] px-2.5 py-1 text-[11px] font-semibold text-[#ea580c]">
                    Use template
                  </span>
                </button>
              ))}
            ...
            <div className="mt-6 flex flex-wrap items-center justify-center gap-2.5">
              ...
              <Link
                ...
                className="inline-flex items-center rounded-2xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:bg-gray-50"
              >
                Manage watchlist
              </Link>
              <Link
