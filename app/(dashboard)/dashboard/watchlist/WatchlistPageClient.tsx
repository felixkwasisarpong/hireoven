...
export default function WatchlistPageClient({
  ...
  const { watchlist, isLoading, addCompany, removeCompany, isWatching } =
    useWatchlist(user?.id, {
      authLoading,
      initialWatchlist,
      initialLoaded: initialWatchlistLoaded,
    })
  const [query, setQuery] = useState("")
  const [discoverCompanies, setDiscoverCompanies] = useState<Company[]>(initialDiscoverCompanies)
  const [insights, setInsights] = useState<Record<string, CompanyInsights>>(initialInsights)
  const [insightsLoading, setInsightsLoading] = useState(false)
  const skipInitialSuggestionsFetchRef = useRef(initialDiscoverLoaded)
  const initialInsightKeyRef = useRef(
    initialInsightsLoaded
      ? watchlist
  ...
  useEffect(() => {
    if (!query.trim() && skipInitialSuggestionsFetchRef.current) {
      skipInitialSuggestionsFetchRef.current = false
      return
    }
  ...
  useEffect(() => {
    const watchlistKey = watchlist
      ...
      .sort()
      .join(",")
    if (watchlistKey && initialInsightKeyRef.current === watchlistKey) {
      initialInsightKeyRef.current = ""
              ...
              <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-gray-200 bg-gray-50/30 py-20 text-center">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-400">
                    Start with quick picks below, then open a company to follow job activity.
                  </p>
                </div>
                {filteredSuggestions.length > 0 && (
                  <div className="w-full max-w-2xl space-y-2 px-4">
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                      {filteredSuggestions.slice(0, 4).map((company) => (
                        <button
                          key={company.id}
                          type="button"
                          onClick={() => void addCompany(company.id)}
                          className="group flex items-center gap-3 rounded-2xl border border-gray-200 bg-white px-3 py-2.5 text-left shadow-sm transition hover:border-blue-200 hover:bg-blue-50/30"
                        >
                          <CompanyLogo
                            companyName={company.name}
                            domain={company.domain}
                            logoUrl={company.logo_url}
                            className="h-8 w-8 flex-shrink-0 rounded-lg border border-slate-100 bg-white object-contain p-0.5"
                          />
                          ...
                          <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg border border-gray-200 bg-white text-gray-400 transition group-hover:border-blue-200 group-hover:text-blue-600">
                  ...
                  <p className="px-4 py-8 text-center text-xs text-gray-400">
                    {query.trim()
                      ...
                      : watchlist.length === 0
                        ...
                        : "All top companies already added!"}
                  </p>
                ) : (
