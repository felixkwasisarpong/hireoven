...
export default function SearchPageClient({
  ...
  const normalizedRawQ = rawQ.trim()
  const canHydrateFromServer =
    initialLoaded &&
    normalizedInitialQuery.length > 0 &&
    normalizedInitialQuery === normalizedRawQ

  const { user } = useAuth()
  const { addCompany, removeCompany, isWatching } = useWatchlist(user?.id)

  const [inputVal, setInputVal] = useState(rawQ)
  const [jobs, setJobs] = useState<JobWithCompany[]>(canHydrateFromServer ? initialJobs : [])
  const [companies, setCompanies] = useState<Company[]>(canHydrateFromServer ? initialCompanies : [])
  const [jobTotal, setJobTotal] = useState(canHydrateFromServer ? initialJobTotal : 0)
  const [isLoading, setIsLoading] = useState(Boolean(normalizedRawQ) && !canHydrateFromServer)
  const inputRef = useRef<HTMLInputElement>(null)
  const hydratedQueryRef = useRef<string>(canHydrateFromServer ? normalizedRawQ : "")
  ...
  useEffect(() => {
    ...
    if (hydratedQueryRef.current && hydratedQueryRef.current === normalizedRawQ) {
      hydratedQueryRef.current = ""
      setIsLoading(false)
      return
    }
    ...
    Promise.all([searchJobs(rawQ, {}, 20, 0), searchCompanies(rawQ, 10)])
          ...
          {rawQ ? (
            <p className="mt-4 text-sm text-muted-foreground">
              {isLoading ? (
                ...
                <>
                  <span className="font-semibold text-strong">{jobTotal.toLocaleString()} jobs</span> and{" "}
                  <span className="font-semibold text-strong">{companies.length.toLocaleString()} companies</span> match
                  &ldquo;{rawQ}&rdquo;
                </>
              )}
            </p>
            ...
            <div className="mt-5 space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {TRENDING.map((term) => (
                  <button key={term} type="button" onClick={() => submit(term)} className="chip-control">
                    {term}
                  </button>
                ))}
              </div>
