...
export default function MatchesPageClient({
  ...
  const [jobs, setJobs] = useState<JobWithMatchScore[]>(
    initialJobsLoaded && initialThreshold === 70 ? initialJobs : []
  )
  const [isLoading, setIsLoading] = useState(Boolean(initialJobsLoaded && initialThreshold === 70 ? false : true))
  const [notifyEnabled, setNotifyEnabled] = useState(initialNotifyLoaded ? initialNotifyEnabled : false)
  const [notifyLoaded, setNotifyLoaded] = useState(initialNotifyLoaded)
  const [isSavingNotify, setIsSavingNotify] = useState(false)
  const skipInitialMatchesFetchRef = useRef(
    initialHasParsedResume && initialJobsLoaded && initialThreshold === 70
  )
