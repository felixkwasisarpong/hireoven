...
export default function OnboardingPage() {
  ...
  async function handleFinish() {
    ...
    try {
      ...
      if (!profileRes.ok) {
        setSaveError("Could not save your setup right now. Please try again.")
        return
      }
        ...
        <div className="w-full max-w-3xl">
          ...
          <div className="mb-8 text-center">
            ...
            <p className="mt-2 text-sm text-gray-500 sm:text-[15px]">
              A quick three-step setup so we can surface the right roles the moment they post, with H-1B intel, cohort signals, and fair-chance context built in.
            </p>
            <div className="mx-auto mt-5 grid max-w-2xl gap-2.5 text-left sm:grid-cols-3">
              <div className="rounded-xl border border-[#D8E8FF] bg-[#F5FAFF] px-3.5 py-3">
                <div className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-white text-[#2563EB]">
                  <Globe2 className="h-3.5 w-3.5" />
                </div>
                <p className="mt-2 text-[12px] font-semibold text-slate-900">H-1B Intel</p>
                <p className="mt-0.5 text-[11px] leading-5 text-slate-600">
                  Petition history and sponsorship confidence on matching jobs.
                </p>
              </div>
              <div className="rounded-xl border border-[#E8DBFF] bg-[#FAF7FF] px-3.5 py-3">
                <div className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-white text-[#7C3AED]">
                  <Users className="h-3.5 w-3.5" />
                </div>
                <p className="mt-2 text-[12px] font-semibold text-slate-900">Cohorts</p>
                <p className="mt-0.5 text-[11px] leading-5 text-slate-600">
                  Compare against similar candidates and sharpen your search strategy.
                </p>
              </div>
              <div className="rounded-xl border border-[#D7F2E2] bg-[#F3FBF6] px-3.5 py-3">
                <div className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-white text-[#047857]">
                  <ShieldCheck className="h-3.5 w-3.5" />
                </div>
                <p className="mt-2 text-[12px] font-semibold text-slate-900">Fair Chance</p>
                <p className="mt-0.5 text-[11px] leading-5 text-slate-600">
                  Identify fair-chance friendly employers and reduce avoidable risk.
                </p>
              </div>
            </div>
          </div>
