...
export type TurnData = { id: string; role: string; content: string; turnIndex: number }

export type DebriefPageClientProps = {
  sessionId: string
  ...
  initialJobCompany: string | null
  initialFinalCode: string | null
...
export default function DebriefPageClient({
  sessionId,
  initialLoaded,
  initialDebrief,
  initialSession,
  ...
  const [debrief, setDebrief] = useState<DebriefData | null>(initialDebrief)
  ...
  const [jobCompany, setJobCompany] = useState<string | null>(initialJobCompany)
  const [finalCode, setFinalCode] = useState<string | null>(initialFinalCode)
    ...
    async function load() {
      try {
        ...
        if (turnsRes.ok) {
          ...
          setTurns(t.turns ?? [])
          setSession((prev) => prev ?? t.session)
          ...
          if (t.session?.type === "coding") {
            ...
            if (snapRes.ok) {
              ...
              setFinalCode((prev) => prev ?? snapData.finalCode ?? null)
              if (snapData.language) {
                setLanguage((prev) => prev ?? snapData.language)
              }
            }
          }
        }
            ...
            <div className="space-y-2.5">
              {(d.gaps as Gap[]).map((g, i) => (
                <div key={i} className="rounded-xl border border-orange-100 bg-white p-4 shadow-sm">
                  ...
                  {g.quote && <p className="mt-2 pl-7 text-[12px] italic leading-relaxed text-slate-500">&ldquo;{g.quote}&rdquo;</p>}
                  {g.suggestion && <p className="mt-2 pl-7 text-[12px] text-slate-700"><span className="font-semibold text-orange-600">Fix: </span>{g.suggestion}</p>}
                </div>
              ))}
            </div>
          </section>
