...
type ResumeScorePageClientProps = {
  initialPrimaryResume?: Resume | null
  initialLoaded?: boolean
}

export default function ResumeScorePageClient({
  initialPrimaryResume = null,
  initialLoaded = false,
}: ResumeScorePageClientProps) {
  ...
  const breakdown = useMemo<ResumeScoreBreakdown | null>(() => {
    if (!resolvedPrimaryResume || resolvedPrimaryResume.parse_status !== "complete") return null
    return buildExtendedBreakdown(resolvedPrimaryResume)
  ...
  return (
    <main className="min-h-[calc(100vh-8.5rem)] bg-[#FAFBFF]">
      <div className="w-full max-w-none space-y-3 px-4 py-3 sm:px-6 lg:px-8 xl:px-10 2xl:px-12">
        ...
        {!resolvedIsLoading && resolvedPrimaryResume && resolvedPrimaryResume.parse_status !== "complete" && (
        ...
        {!resolvedIsLoading && breakdown && resolvedPrimaryResume && (
          <div className="space-y-3">
            ...
            <div className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-3 sm:grid-cols-[1fr_auto] sm:items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-900">{resolvedPrimaryResume.name ?? resolvedPrimaryResume.file_name}</p>
                  <p className="mt-0.5 text-[12.5px] text-slate-500">{resolvedPrimaryResume.primary_role ?? "Role not detected"}</p>
                </div>
              </div>
            ...
            <section id="detailed-insights" className="rounded-2xl border border-slate-200 bg-white p-4">
              ...
              <div className="grid max-h-[calc(100vh-34rem)] min-h-[160px] gap-3 overflow-y-auto lg:grid-cols-2">
                {breakdown.categories
                  ...
                  .slice(0, 4)
                  .map((cat) => (
                    <DetailedInsight key={cat.key} resume={resolvedPrimaryResume} category={cat} />
                  ))}
              </div>
            </section>

            <InternationalPanel resume={resolvedPrimaryResume} />
