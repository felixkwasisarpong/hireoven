...
export default async function DashboardJobDetailPage({ params, searchParams }: Props) {
  ...
  const DISPLAY_NOISE_RE =
    /\b(travel\s+\d|requires?\s+travel|travel\s+required|\d+%\s+of\s+the\s+time|performed\s+in\s+an\s+office|office\s+setting|sit\s+and\s+stand|prolonged\s+(?:sitting|standing|computer\s+use|walking|driving|typing)|standard\s+office\s+equipment|standard\s+office\s+working\s+conditions|incumbent|mental\/physical|physical\s+requirements?|physical\s+demands?|lift(?:ing)?\s+\d|sedentary|varies?\s+(?:upon|depending\s+on)\s+the\s+needs\s+of\s+the\s+department|may\s+vary\s+depending\s+on\s+job[- ]related\s+factors|work\s+hours?|the\s+employee\s+is\s+required|while\s+performing\s+the\s+duties|essential\s+functions\s+of\s+this\s+job|reasonable\s+accommodations\s+may\s+be\s+made|automated\s+tools?\s+to\s+support.{0,60}recruiting\s+process|domestic\s+partners?.{0,60}eligible\s+to\s+participate)\b/i
