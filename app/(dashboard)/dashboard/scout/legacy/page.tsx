...
export default function ScoutLegacyPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/scout/strategy", { cache: "no-store", headers: { Accept: "application/json" } })
      .then(async (res) => {
        const data = (await res.json().catch(() => ({}))) as { board?: ScoutStrategyBoard; error?: string } | undefined
        if (!res.ok) throw new Error(data?.error ?? "Failed to load strategy board")
        if (!cancelled) setStrategyBoard(data?.board ?? null)
      })
      .catch((err) => { if (!cancelled) setStrategyError(err instanceof Error ? err.message : "Failed to load strategy board") })
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    setMessages((prev) => [...prev, { id: `u-${Date.now()}`, role: "user", text: trimmedQuery }])
    setQuery("")
    setIsLoading(true)
    setError(null)
    try {
      const res = await fetch("/api/scout/chat", {
        ...
        body: JSON.stringify({ message: trimmedQuery, pagePath: pathname, commandMode: isCommandMode, focusMode: searchParams.get("focus") === "1", activeFilters: { q: searchParams.get("q") ?? undefined, location: searchParams.get("location") ?? undefined, sponsorship: searchParams.get("sponsorship") ?? undefined, workMode: searchParams.get("workMode") ?? undefined }, ...contextIds }),
      })
      const raw = (await res.json().catch(() => null)) as unknown
      if (!res.ok) {
        setError(typeof raw === "object" && raw !== null && "error" in (raw as Record<string, unknown>) ? String((raw as Record<string, unknown>).error) : "Failed to get response from Scout")
        return
      }
  ...
  function fillChip(chip: string) { setQuery(chip); setActiveTab("overview") }
  ...
  async function runScoutCommand(message: string, tab: Tab) {
    ...
    try {
      const res = await fetch("/api/scout/chat", {
        method: "POST",
        headers: { Accept: "application/json", "Content-Type": "application/json" },
        body: JSON.stringify({ message, pagePath: pathname, commandMode: true, focusMode: false, activeFilters: { q: searchParams.get("q") ?? undefined, location: searchParams.get("location") ?? undefined, sponsorship: searchParams.get("sponsorship") ?? undefined, workMode: searchParams.get("workMode") ?? undefined }, ...contextIds }),
      })
      const raw = (await res.json().catch(() => null)) as unknown
      if (!res.ok) { setCompareError(typeof raw === "object" && raw !== null && "error" in (raw as Record<string, unknown>) ? String((raw as Record<string, unknown>).error) : "Scout couldn't run that command."); return }
      const normalized = normalizeScoutResponse(raw)
      setMessages((prev) => [...prev, { id: `u-${Date.now()}-cmd`, role: "user", text: message }, { id: `s-${Date.now()}-cmd`, role: "scout", response: normalized }])
    } catch {
  ...
  return (
    <main className="app-page">
      {/* Back to workspace */}
      <div className="border-b border-gray-100 bg-white px-5 py-3 sm:px-7">
        <Link
        ...
        <div className="overflow-x-auto scrollbar-none">
          <nav aria-label="Scout sections" className="flex items-stretch gap-0 px-4 sm:px-7">
            {TABS.map(({ id, label, Icon }) => {
              ...
              const hasDot = (id === "compare" && lastCompareResponse !== null) || (id === "interview-prep" && lastInterviewPrepResponse !== null) || (id === "actions" && (lastWorkflowResponse !== null || lastActionsResponse !== null))
              return (
                <button key={id} type="button" onClick={() => setActiveTab(id)} aria-current={active ? "page" : undefined}
                  className={`group relative flex shrink-0 items-center gap-2 border-b-2 px-3 py-3 text-[13px] font-medium transition-colors sm:px-4 ${active ? "border-[#ea580c] bg-[#ea580c]/[0.04] text-[#ea580c]" : "border-transparent text-slate-600 hover:bg-slate-50/80 hover:text-slate-900"}`}>
                  {active && <span className="absolute left-0 top-1/2 hidden h-5 w-0.5 -translate-y-1/2 rounded-full bg-[#ea580c] sm:block" aria-hidden />}
                  <span className={`flex h-7 w-7 items-center justify-center rounded-md border transition-colors ${active ? "border-[#ea580c]/20 bg-white text-[#ea580c]" : "border-slate-200/80 bg-slate-50 text-slate-500 group-hover:border-slate-300 group-hover:text-slate-700"}`}>
                    <Icon className="h-3.5 w-3.5" aria-hidden />
                  </span>
                  {label}
                  {hasDot && !active && <span className="h-1.5 w-1.5 flex-shrink-0 rounded-full bg-blue-500" />}
                </button>
              )
            })}
          </nav>
      ...
      <div className="app-shell w-full max-w-3xl py-6 pb-12">
        {activeTab === "overview" && (
          <ScoutOverviewTab greeting={greeting} firstName={firstName} query={query} setQuery={setQuery} isLoading={isLoading} isCommandMode={isCommandMode} setIsCommandMode={setIsCommandMode} suggestionChips={suggestionChips} messages={messages} error={error} resumeRefreshedNotice={resumeRefreshedNotice} strategyBoard={strategyBoard} strategyLoading={strategyLoading} nudges={nudges} hasConversation={hasConversation} userTurns={userTurns} onSubmit={handleSubmit} onFillChip={fillChip} onClearChat={() => { setMessages([]); setError(null) }} onResetContext={() => { setMessages([]); setError(null) }} onViewStrategy={() => setActiveTab("strategy")} />
        )}
        {activeTab === "strategy" && <ScoutStrategyTab board={strategyBoard} isLoading={strategyLoading} error={strategyError} behaviorSignals={behaviorSignals} behaviorLoading={behaviorLoading} />}
        {activeTab === "actions" && <ScoutActionsTab lastWorkflowResponse={lastWorkflowResponse} lastActionsResponse={lastActionsResponse} onFillChip={fillChip} />}
        {activeTab === "compare" && <ScoutCompareTab compareResponse={lastCompareResponse} onRunCompareCommand={(msg) => runScoutCommand(msg, "compare")} isLoading={compareLoading} error={compareError} />}
        {activeTab === "interview-prep" && <ScoutInterviewPrepTab interviewPrep={lastInterviewPrepResponse} onFillChip={fillChip} />}
