...
function formFromAutofillProfile(profile: AutofillProfile): FormState {
...
export default function AutofillPageClient({
  initialAutofillProfile = null,
  initialLoaded = false,
}: AutofillPageClientProps) {
  const { profile: authProfile } = useAuth()
  const { primaryResume } = useResumeContext()
  const [form, setForm] = useState<FormState>(
    initialAutofillProfile ? formFromAutofillProfile(initialAutofillProfile) : EMPTY_FORM
  )
  const [profileId, setProfileId] = useState<string | null>(initialAutofillProfile?.id ?? null)
  ...
  useEffect(() => {
    function seedFromResumeOrAuth() {
      if (primaryResume?.parse_status === "complete") {
    ...
    async function load() {
      if (initialLoaded) {
        if (initialAutofillProfile) {
          setProfileId(initialAutofillProfile.id)
          setForm(formFromAutofillProfile(initialAutofillProfile))
          return
        }
        seedFromResumeOrAuth()
        return
      }
      ...
      if (autofillProfile) {
        ...
        setForm(formFromAutofillProfile(autofillProfile))
        return
      }
    ...
    void load()
  }, [authProfile, primaryResume, initialAutofillProfile, initialLoaded])
