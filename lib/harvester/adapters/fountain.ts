...
/**
 * Fountain adapter (high-volume hourly hiring).
 *
 * Fountain powers branded hourly-hiring career pages. Each customer is a
 * separate "company"; the tenant slug is the company token that appears in the
 * Fountain careers host/path, e.g.
 *   https://acme.fountain.com
 *   https://app.fountain.com/careers/acme
 * detectFromUrl() round-trips that token back to the slug.
 *
 * ┌────────────────────────────────────────────────────────────────────────┐
 * │ VERIFY LIVE ENDPOINT before enabling in harvest rotation.                │
 * │ Fountain's public listing JSON shape is NOT verified here. The field     │
 ...
 * │ company token and set FOUNTAIN_API_BASE / FOUNTAIN_JOBS_PATH if a tenant │
 * │ deviates. All parsing is unit-tested against mocked JSON only.           │
 * └────────────────────────────────────────────────────────────────────────┘
 *
 * API surface (defaults, overridable):
 *   GET {FOUNTAIN_API_BASE}/careers/{company}/positions.json?page={page}
 */

const DEFAULT_API_BASE = "https://app.fountain.com"
const API_BASE = (process.env.FOUNTAIN_API_BASE?.trim() || DEFAULT_API_BASE).replace(/\/+$/, "")
const JOBS_PATH =
  process.env.FOUNTAIN_JOBS_PATH?.trim() || "/careers/{company}/positions.json?page={page}"

const RESULTS_PER_PAGE = Math.max(
  10,
  Math.min(200, Number.parseInt(process.env.HARVESTER_FOUNTAIN_PAGE_SIZE ?? "100", 10))
)
const MAX_PAGES = Math.max(
  1,
  Number.parseInt(process.env.HARVESTER_FOUNTAIN_MAX_PAGES ?? "20", 10)
)
const DEFAULT_TIMEOUT_MS = 15_000
const RETRY_STATUSES = new Set([408, 425, 429, 500, 502, 503, 504])

const FOUNTAIN_HOST_RE = /(^|\.)fountain\.com$/
// Company token: letters / digits / dash / underscore, 2–64 chars.
...
function detectFromUrl(url: string): { slug: string } | null {
  ...
  if (!FOUNTAIN_HOST_RE.test(host)) return null
  const parts = parsed.pathname.split("/").filter(Boolean)
  // app.fountain.com/careers/<company>  (and www./bare fountain.com)
  if (parts[0]?.toLowerCase() === "careers" && parts[1]) {
    const slug = decodeURIComponent(parts[1])
    return SLUG_RE.test(slug) ? { slug } : null
  }
  // <company>.fountain.com — first subdomain label is the company token.
  const label = host.split(".")[0]
  if (label && label !== "app" && label !== "www" && host !== "fountain.com") {
...
function applyUrlFor(slug: string, jobId: string): string {
  return `${API_BASE}/careers/${encodeURIComponent(slug)}/apply/${encodeURIComponent(jobId)}`
}
...
type FountainResponse =
  ...
  | {
      ...
      items?: FountainJob[]
      jobs?: FountainJob[]
      positions?: FountainJob[]
      postings?: FountainJob[]
      opportunities?: FountainJob[]
      total?: number
      totalCount?: number
      count?: number
    }
...
function extractJobs(data: FountainResponse): { jobs: FountainJob[]; total: number } {
...
export function mapItemToJob(slug: string, item: FountainJob): HarvestedJob | null {
  ...
  return {
    externalId: `fountain:${slug}:${externalIdRaw}`,
    title,
    applyUrl,
    description,
    location,
...
async function fetchPage(
  ...
  | { ok: true; data: FountainResponse; upstreamLatencyMs: number }
...
> {
  ...
  while (attempt < 3) {
    ...
    try {
      ...
      if (response.ok) {
        const data = (await response.json()) as FountainResponse
...
export const fountainAdapter: AtsAdapter = {
  name: "fountain",
  concurrency: envConcurrency("fountain", 3),
  detectFromUrl,
  async fetchJobs({ slug, ctx }): Promise<HarvestResult> {
    const fetchedAt = new Date()
    const startedAt = Date.now()
    if (!SLUG_RE.test(slug)) throw new Error(`fountain malformed slug: ${slug}`)
    ...
    for (let page = 1; page <= MAX_PAGES; page++) {
      ...
      if (!result.ok) {
        if (page === 1) {
          const err = new Error(`fountain fetch failed: ${result.reason}`)
    ...
    if (pagesFetched === 0) {
      const err = new Error("fountain fetch failed: no pages fetched")
