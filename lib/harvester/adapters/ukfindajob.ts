...
/**
 * UK "Find a Job" (DWP) aggregator adapter.
 *
 * "Find a Job" is the UK Department for Work and Pensions public job-posting
 * service (findajob.dwp.gov.uk). We treat each search facet (a keyword,
 * sector or region token) as a separate "company"; the tenant slug is that
 * facet token.
 *
 * The seed/discovery channel creates one company per facet with a synthetic
 * careers URL of the form
 *   https://findajob.dwp.gov.uk/search?q=<slug>
 * which detectFromUrl() round-trips back to the slug.
 *
 * API surface
 * -----------
 *   GET {UKFINDAJOB_API_BASE}{UKFINDAJOB_JOBS_PATH}
 *   default: GET https://findajob.dwp.gov.uk/api/jobs?q=<slug>&page=<N>
 ...
 * --------------------------------------------------------------------
 * "Find a Job" is a server-rendered service without a documented public JSON
 * API; the base path and field names below are ASSUMED and were NOT verifiable
 * at authoring time. `mapItemToJob` is deliberately defensive (multiple
 * fallbacks per field) and the response envelope is read from any of the
 * common keys. Verify against one live facet token before enabling, and
 * override the endpoint with:
 *   UKFINDAJOB_API_BASE   (default https://findajob.dwp.gov.uk)
 *   UKFINDAJOB_JOBS_PATH  (default /api/jobs?q={q}&page={page})
 * All parsing is unit-tested against mocked JSON.
 */

const DEFAULT_API_BASE = "https://findajob.dwp.gov.uk"
const API_BASE = (process.env.UKFINDAJOB_API_BASE?.trim() || DEFAULT_API_BASE).replace(/\/+$/, "")
// `{q}` and `{page}` placeholders are substituted at request time.
const JOBS_PATH = process.env.UKFINDAJOB_JOBS_PATH?.trim() || "/api/jobs?q={q}&page={page}"

const RESULTS_PER_PAGE = Math.max(
  10,
  Math.min(100, Number.parseInt(process.env.HARVESTER_UKFINDAJOB_PAGE_SIZE ?? "50", 10))
)
const MAX_PAGES = Math.max(
  1,
  Number.parseInt(process.env.HARVESTER_UKFINDAJOB_MAX_PAGES ?? "20", 10)
)
const DEFAULT_TIMEOUT_MS = 15_000
const RETRY_STATUSES = new Set([408, 425, 429, 500, 502, 503, 504])

const SYNTHETIC_HOST = "findajob.dwp.gov.uk"
...
function applyUrlFor(jobId: string): string {
  return `${API_BASE}/details/${encodeURIComponent(jobId)}`
}
...
type UkFindAJob = {
  ...
  vacancyReference?: string | number
  // title
  title?: string
  jobTitle?: string
  name?: string
...
function toSalary(
  ...
  return { min: lo, max: hi, currency: currency?.trim() || "GBP" }
}

function locationFor(item: UkFindAJob): string | undefined {
  const direct = firstString(item.locationName, item.location)
  if (direct) return direct
  const parts = [item.town, item.region].map((p) => p?.trim()).filter((p): p is string => Boolean(p))
  return parts.length ? parts.join(", ") : undefined
}

function extractJobs(data: UkFindAJobResponse): { jobs: UkFindAJob[]; total: number } {
...
export function mapItemToJob(slug: string, item: UkFindAJob): HarvestedJob | null {
  const title = firstString(item.title, item.jobTitle, item.name)
  if (!title) return null
  const externalIdRaw = firstString(item.id, item.jobId, item.reference, item.vacancyReference)
  ...
  const employmentType = firstString(item.contractType, item.employmentType, item.jobType, item.hours)
  ...
  return {
    externalId: `ukfindajob:${slug}:${externalIdRaw}`,
    title,
    applyUrl,
    description,
    location,
...
async function fetchPage(
  ...
  | { ok: true; data: UkFindAJobResponse; upstreamLatencyMs: number }
...
> {
  ...
  while (attempt < 3) {
    ...
    try {
      ...
      if (response.ok) {
        const data = (await response.json()) as UkFindAJobResponse
...
export const ukfindajobAdapter: AtsAdapter = {
  name: "ukfindajob" as AtsAdapter["name"],
  // One central API host (findajob.dwp.gov.uk) shared by every facet — keep
  // the per-process budget modest so co-scheduled facets don't hammer it.
  concurrency: envConcurrency("ukfindajob" as AtsAdapter["name"], 2),
  detectFromUrl,
  async fetchJobs({ slug, ctx }): Promise<HarvestResult> {
    const fetchedAt = new Date()
    const startedAt = Date.now()
    if (!SLUG_RE.test(slug)) throw new Error(`ukfindajob malformed slug: ${slug}`)
    ...
    for (let page = 1; page <= MAX_PAGES; page++) {
      ...
      if (!result.ok) {
        if (page === 1) {
          const err = new Error(`ukfindajob fetch failed: ${result.reason}`)
    ...
    if (pagesFetched === 0) {
      const err = new Error("ukfindajob fetch failed: no pages fetched")
    ...
    return {
      ...
      etag: null,
      lastModified: null,
      sourceAts: "ukfindajob" as HarvestResult["sourceAts"],
      sourceAtsSlug: slug,
      fetchedAt,
