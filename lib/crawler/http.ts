...
export function detectBlockedHtml(html: string | null | undefined): string | null {
  ...
  const hasCloudflareBlockSignal =
    ...
    (
      compact.includes("attention required!") ||
      compact.includes("checking your browser") ||
      compact.includes("cf-browser-verification") ||
      compact.includes("cloudflare ray id") ||
      compact.includes("error 1020")
    )

  // Some valid enterprise sites (including careers pages) embed Akamai
  // telemetry snippets in normal HTML. Treat Akamai as blocked only when
  // strong block-page markers are present.
  const hasAkamaiBlockSignal =
    compact.includes("akamai") &&
    (
      compact.includes("reference #") ||
      compact.includes("error reference number") ||
      compact.includes("you don't have permission to access") ||
      compact.includes("you dont have permission to access") ||
      compact.includes("generated by akamai") ||
      compact.includes("akamai bot manager")
    )

  const hasIncapsulaBlockSignal =
    compact.includes("incapsula") &&
    (
      compact.includes("request unsuccessful") ||
      compact.includes("incap_ses") ||
      compact.includes("_incapsula_resource") ||
      compact.includes("powered by incapsula")
    )

  const hasPerimeterXBlockSignal =
    compact.includes("perimeterx") &&
    (
      compact.includes("press and hold") ||
      compact.includes("verify you are human") ||
      compact.includes("px-captcha") ||
  ...
  if (hasAkamaiBlockSignal) return "blocked_html_akamai"
  if (hasIncapsulaBlockSignal) return "blocked_html_incapsula"
  if (hasPerimeterXBlockSignal) return "blocked_html_perimeterx"
