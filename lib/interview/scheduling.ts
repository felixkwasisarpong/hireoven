...
export async function suggestSlotsForDay(input: {
  ...
  const [bookedResult, historyResult] = await Promise.all([
    ...
    pool.query<{ hour: number; count: string }>(
      `SELECT EXTRACT(HOUR FROM started_at)::int AS hour, COUNT(*)::text AS count
       FROM interview_sessions
       WHERE type = 'live'
...
export async function setSessionSchedule(
  ...
  await pool.query(
    ...
    [sessionId, userId, scheduledAt.toISOString(), timeZone],
  )
}
...
export async function resetRemindersForSession(
  ...
  const now = Date.now()
  for (const { kind, minutesBefore } of REMINDER_OFFSETS) {
    const remindAt = new Date(scheduledAt.getTime() - minutesBefore * 60_000)
    ...
    await pool.query(
      ...
      [sessionId, userId, kind, remindAt.toISOString()],
    )
  }
}

export async function clearRemindersForSession(sessionId: string): Promise<void> {
  const pool = getPostgresPool()
  await pool.query(
    `DELETE FROM interview_reminders WHERE session_id = $1 AND sent_at IS NULL`,
    [sessionId],
...
export function buildInterviewIcs(input: {
  ...
  const lines = [
    ...
    `DTSTART:${icsUtcStamp(input.scheduledAt)}`,
